...
class ImitationTask(object):
  ...
  def _calc_reward_end_effector(self):
    ...
    if issubclass(self._env.robot_class, go1.Go1):
      return self._calc_reward_end_effector_Go1()
    if self._using_real_robot:
      raise NotImplementedError(
          "End effector positions unknown for real robots other than Go1")
    return self._calc_reward_end_effector_sim()

  def _calc_reward_end_effector_Go1(self):
    """Get the end effector reward for sim or real Go1 robot."""
    ref_joint_angles = self.get_active_motion().get_frame_joints(self._ref_pose)
